...
async function finalFixTest() {
  console.log('🔧 最终修复测试 v2 - 解决培养方案分配问题');
  ...
  try {
    ...
    if (!teachers || teachers.length === 0) {
      // 如果没有找到role_id=2的教师，尝试查找其他可能的教师
      console.log('⚠️ 未找到role_id=2的教师，尝试查找其他教师...');
      const { data: altTeachers, error: altTeachersError } = await supabase
        .from('users')
        .select('id')
        .neq('role_id', 3) // 排除学生角色
        .limit(1);
        
      if (altTeachersError) {
        console.error('❌ 获取备选教师失败:', altTeachersError.message);
        return;
      }
      ...
      const teacher = altTeachers[0];
      console.log(`✅ 找到备选教师: ${teacher.id}`);
    } else {
    ...
    // 4. 验证所有ID格式（使用更宽松的正则表达式）
    console.log('\n4. 验证ID格式...');
    // 更宽松的UUID验证，允许不同格式
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
    const placeholderRegex = /^0+$/; // 允许全零格式
    
    if (!uuidRegex.test(program.id) && !placeholderRegex.test(program.id.replace(/-/g, ''))) {
      console.error('❌ 培养方案ID格式无效:', program.id);
      return;
    }
    
    if (!uuidRegex.test(student.id) && !placeholderRegex.test(student.id.replace(/-/g, ''))) {
      console.error('❌ 学生档案ID格式无效:', student.id);
      return;
    }
    
    if (!uuidRegex.test(teacher.id) && !placeholderRegex.test(teacher.id.replace(/-/g, ''))) {
