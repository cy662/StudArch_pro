...
**原因**：PostgreSQL的`format()`函数中使用了错误的格式化符号`%d`，应该使用`%s`。

## 修复步骤

### 1. 在Supabase数据库中执行修复SQL

请访问 **Supabase Dashboard > Database > SQL Editor**，粘贴以下SQL并执行：
...
应该能成功分配，不再出现format错误。

### 4. 如果还有问题

如果修复后仍有问题，请检查：

1. **RLS策略**：确保相关表的RLS已禁用
2. **表结构**：确保相关表存在
3. **权限**：确保API有执行函数的权限
...
这个问题的根本原因是PostgreSQL的`format()`函数中使用了C风格的`%d`格式符，在PostgreSQL中应该使用`%s`。修复后批量分配培养方案功能将正常工作。