-- 学生教学任务与安排相关数据库设计（安全版本）
-- 包含：技术标签、学习收获、学习成果、证明材料

-- ==================== 首先检查现有student_profiles表结构 ====================
DO $$
DECLARE
    v_has_full_name BOOLEAN := FALSE;
    v_has_student_number BOOLEAN := FALSE;
BEGIN
    -- 检查字段是否存在
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'student_profiles' AND column_name = 'full_name'
    ) INTO v_has_full_name;
    
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'student_profiles' AND column_name = 'student_number'
    ) INTO v_has_student_number;
    
    -- 如果缺少字段则添加
    IF NOT v_has_full_name THEN
        ALTER TABLE student_profiles ADD COLUMN IF NOT EXISTS full_name TEXT;
        RAISE NOTICE '已添加full_name字段到student_profiles表';
    END IF;
    
    IF NOT v_has_student_number THEN
        ALTER TABLE student_profiles ADD COLUMN IF NOT EXISTS student_number TEXT;
        RAISE NOTICE '已添加student_number字段到student_profiles表';
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS student_technical_tags (
    ...
    proficiency_level VARCHAR(30) NOT NULL CHECK (proficiency_level IN ('beginner', 'intermediate', 'advanced', 'expert')), -- 掌握程度
    ...
    -- 状态和审核
    status VARCHAR(30) DEFAULT 'active' CHECK (status IN ('active', 'archived', 'pending', 'rejected')), -- 状态
...
CREATE TABLE IF NOT EXISTS student_learning_achievements (
    ...
    achievement_type VARCHAR(30) NOT NULL CHECK (achievement_type IN ('knowledge', 'skill', 'experience', 'insight', 'other')), -- 收获类型
    ...
    -- 影响评估
    impact_level VARCHAR(30) DEFAULT 'medium' CHECK (impact_level IN ('low', 'medium', 'high', 'significant')), -- 影响程度
...
CREATE TABLE IF NOT EXISTS student_learning_outcomes (
    ...
    outcome_type VARCHAR(30) NOT NULL CHECK (outcome_type IN ('project', 'competition', 'certification', 'research', 'internship', 'other')), -- 成果类型
    
    -- 详细信息
    domain VARCHAR(100), -- 领域（如：Web开发、人工智能、数据分析等）
    difficulty_level VARCHAR(30) DEFAULT 'medium' CHECK (difficulty_level IN ('basic', 'medium', 'advanced', 'expert')), -- 难度等级
    completion_status VARCHAR(30) DEFAULT 'completed' CHECK (completion_status IN ('planning', 'in_progress', 'completed', 'abandoned')), -- 完成状态
...
CREATE TABLE IF NOT EXISTS student_proof_materials (
    ...
    material_type VARCHAR(30) NOT NULL CHECK (material_type IN ('certificate', 'award', 'project_report', 'transcript', 'recommendation', 'work_sample', 'other')), -- 材料类型
    ...
    -- 状态和审核
    verification_status VARCHAR(30) DEFAULT 'pending' CHECK (verification_status IN ('pending', 'verified', 'rejected', 'expired')), -- 验证状态
...
-- ==================== 学习记录综合表（安全版本） ====================
DROP VIEW IF EXISTS student_learning_summary;
...
-- 更新技术标签时间戳
DROP TRIGGER IF EXISTS update_student_technical_tags_updated_at ON student_technical_tags;
CREATE TRIGGER update_student_technical_tags_updated_at 
    BEFORE UPDATE ON student_technical_tags 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 更新学习收获时间戳
DROP TRIGGER IF EXISTS update_student_learning_achievements_updated_at ON student_learning_achievements;
CREATE TRIGGER update_student_learning_achievements_updated_at 
    BEFORE UPDATE ON student_learning_achievements 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 更新学习成果时间戳
DROP TRIGGER IF EXISTS update_student_learning_outcomes_updated_at ON student_learning_outcomes;
CREATE TRIGGER update_student_learning_outcomes_updated_at 
    BEFORE UPDATE ON student_learning_outcomes 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 更新证明材料时间戳
DROP TRIGGER IF EXISTS update_student_proof_materials_updated_at ON student_proof_materials;
CREATE TRIGGER update_student_proof_materials_updated_at 
    BEFORE UPDATE ON student_proof_materials 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 更新标签分类时间戳
DROP TRIGGER IF EXISTS update_technical_tag_categories_updated_at ON technical_tag_categories;
...
CREATE OR REPLACE FUNCTION add_student_technical_tag(
    ...
    p_proficiency_level VARCHAR(30),
...
CREATE OR REPLACE FUNCTION add_learning_achievement(
    ...
    p_impact_level VARCHAR(30) DEFAULT 'medium',
...
CREATE OR REPLACE FUNCTION add_learning_outcome(
    ...
    p_difficulty_level VARCHAR(30) DEFAULT 'medium',
    p_completion_status VARCHAR(30) DEFAULT 'completed',
...
ALTER TABLE technical_tag_categories DISABLE ROW LEVEL SECURITY;

-- ==================== 验证创建结果 ====================
DO $$
BEGIN
    RAISE NOTICE '=== 学生教学任务与安排数据库表创建完成 ===';
    RAISE NOTICE '1. student_technical_tags: % 条记录', (SELECT COUNT(*) FROM student_technical_tags);
    RAISE NOTICE '2. student_learning_achievements: % 条记录', (SELECT COUNT(*) FROM student_learning_achievements);
    RAISE NOTICE '3. student_learning_outcomes: % 条记录', (SELECT COUNT(*) FROM student_learning_outcomes);
    RAISE NOTICE '4. student_proof_materials: % 条记录', (SELECT COUNT(*) FROM student_proof_materials);
    RAISE NOTICE '5. technical_tag_categories: % 条记录', (SELECT COUNT(*) FROM technical_tag_categories);
    RAISE NOTICE '6. student_learning_summary 视图创建成功';
    RAISE NOTICE '=== 所有表结构已就绪，可以开始使用 ===';
END $$;
