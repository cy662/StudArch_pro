...
-- 搜索学生学习记录
CREATE OR REPLACE FUNCTION search_student_learning_records(
    p_search_keyword TEXT,
    p_class_name VARCHAR(100) DEFAULT NULL,
    ...
    student_name VARCHAR(200),
    class_name VARCHAR(100),
    record_type VARCHAR(50), -- 'tag', 'achievement', 'outcome', 'material'
    record_id UUID,
    record_title TEXT,
    record_content TEXT,
    created_at TIMESTAMP WITH TIME ZONE
...
BEGIN
    ...
    SELECT 
        sp.id as student_id,
        COALESCE(sp.full_name, u.full_name, u.username, '未知学生') as student_name,
        COALESCE(sp.class_name, u.class_name, '未分配班级') as class_name,
        'tag' as record_type,
        stt.id as record_id,
        stt.tag_name as record_title,
        COALESCE(stt.description, '') as record_content,
        stt.created_at
    FROM student_profiles sp
    LEFT JOIN users u ON sp.user_id = u.id
    JOIN student_technical_tags stt ON sp.id = stt.student_profile_id
    WHERE (p_class_name IS NULL OR sp.class_name = p_class_name OR u.class_name = p_class_name)
    AND (
        LOWER(stt.tag_name) LIKE '%' || LOWER(p_search_keyword) || '%' OR
        LOWER(COALESCE(stt.description, '')) LIKE '%' || LOWER(p_search_keyword) || '%'
    )
    ...
    SELECT 
        ...
        sla.id as record_id,
        sla.title as record_title,
        sla.content as record_content,
        sla.created_at
    FROM student_profiles sp
    LEFT JOIN users u ON sp.user_id = u.id
    JOIN student_learning_achievements sla ON sp.id = sla.student_profile_id
    WHERE (p_class_name IS NULL OR sp.class_name = p_class_name OR u.class_name = p_class_name)
    AND (
        LOWER(sla.title) LIKE '%' || LOWER(p_search_keyword) || '%' OR
        LOWER(sla.content) LIKE '%' || LOWER(p_search_keyword) || '%'
    )
    ...
    SELECT 
        ...
        slo.id as record_id,
        slo.outcome_title as record_title,
        COALESCE(slo.outcome_description, '') as record_content,
        slo.created_at
    FROM student_profiles sp
    LEFT JOIN users u ON sp.user_id = u.id
    JOIN student_learning_outcomes slo ON sp.id = slo.student_profile_id
    WHERE (p_class_name IS NULL OR sp.class_name = p_class_name OR u.class_name = p_class_name)
    AND (
        LOWER(slo.outcome_title) LIKE '%' || LOWER(p_search_keyword) || '%' OR
        LOWER(COALESCE(slo.outcome_description, '')) LIKE '%' || LOWER(p_search_keyword) || '%'
    )
    ...
    SELECT 
        ...
        COALESCE(sp.full_name, u.full_name, u.username, '未知学生') as student_name,
        COALESCE(sp.class_name, u.class_name, '未分配班级') as class_name,
        'material' as record_type,
        spm.id as record_id,
        spm.material_name as record_title,
        COALESCE(spm.material_description, '') as record_content,
        spm.created_at
    FROM student_profiles sp
    LEFT JOIN users u ON sp.user_id = u.id
    JOIN student_proof_materials spm ON sp.id = spm.student_profile_id
    WHERE (p_class_name IS NULL OR sp.class_name = p_class_name OR u.class_name = p_class_name)
    AND (
        LOWER(spm.material_name) LIKE '%' || LOWER(p_search_keyword) || '%' OR
        LOWER(COALESCE(spm.material_description, '')) LIKE '%' || LOWER(p_search_keyword) || '%'
    )
